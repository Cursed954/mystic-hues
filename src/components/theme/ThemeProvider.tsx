...
type ThemeContextType = {
  ...
  themeLoaded: boolean; // Add loading state flag
};

const ThemeContext = createContext<ThemeContextType | undefined>(undefined);

export function ThemeProvider({ children }: { children: React.ReactNode }) {
  const [theme, setTheme] = useState<Theme>('light'); // Default to light to avoid flash
  ...
  // Apply theme to document and preload assets
  const applyTheme = (activeTheme: Theme) => {
    ...
    darkBg.src = "https://r4.wallpaperflare.com/wallpaper/684/422/438/abstract-3d-digital-art-stu-ballinger-wallpaper-2b965cfd43817fe9f584cbf97d1cfc40.jpg";
    
    const lightBg = new Image();
    lightBg.src = "https://images.unsplash.com/photo-1740137660661-96c8ec7bc92e?q=80&w=2268&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D";
    
    // Apply theme-specific background styles with improved performance
    if (activeTheme === 'dark') {
      document.body.style.setProperty('--theme-bg-image', `url(${darkBg.src})`);
      document.body.classList.remove('light-theme');
      document.body.classList.add('dark-theme');
    } else {
      document.body.style.setProperty('--theme-bg-image', `url(${lightBg.src})`);
